...
def prepare_babi_data(data_dir, vocabulary_size, tokenizer=None):
    ...
    vocab_path = os.path.join(data_dir, "vocab%d.dat" % vocabulary_size)
    create_vocabulary(vocab_path, TRAIN, vocabulary_size, tokenizer)

    # Create token ids for the training data.
    tgt_train_ids_path = TRAIN + (".ids%d.tgt" % vocabulary_size)
    src_train_ids_path = TRAIN + (".ids%d.src" % vocabulary_size)
    data_to_token_ids(train_path + ".tgt", tgt_train_ids_path, vocab_path, tokenizer)
    data_to_token_ids(train_path + ".src", src_train_ids_path, target_vocab_path, tokenizer)

    # Create token ids for the development data.
    tgt_dev_ids_path = dev_path + (".ids%d.fr" % fr_vocabulary_size)
    src_dev_ids_path = dev_path + (".ids%d.en" % vocabulary_size)
    data_to_token_ids(dev_path + ".fr", tgt_dev_ids_path, vocab_path, tokenizer)
    data_to_token_ids(dev_path + ".en", src_dev_ids_path, target_vocab_path, tokenizer)

    return (src_train_ids_path, tgt_train_ids_path,
            src_dev_ids_path, tgt_dev_ids_path,
            target_vocab_path, vocab_path)
